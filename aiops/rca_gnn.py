...
def build_incident_edge_index(
    ...
    Returns ``(edge_index, edge_weight)``.  The weight is the entity-graph
    evidence confidence for that link, so the RCA GNN attends more to links the
    control plane confirms than to links only implied by a routed prefix.
    """
    empty = (
    ...
    for i, (node, inc) in enumerate(zip(rca_features["network_element_id"].astype(str), inc_col)):
        if node not in entity_graph:
            continue
        # Union of successors and predecessors: the incident graph is treated as
        # undirected so a root cause can propagate either way along a link.
        peers = set(entity_graph.successors(node)) if entity_graph.is_directed() else set(entity_graph.neighbors(node))
        if entity_graph.is_directed():
            peers |= set(entity_graph.predecessors(node))
        for peer in peers:
            j = id_to_idx.get(str(peer))
            if j is None or j == i or inc_col[j] != inc:
                continue
            conf = 1.0
            try:
                conf = float(entity_graph[node][peer].get("confidence", 1.0))
            except Exception:
                conf = 1.0
            for k in ((i, j), (j, i)):
                edges[k] = max(conf, edges.get(k, 0.0))
    if not edges:
        return empty
