...
def _role_one_hot(node: str) -> dict[str, float]:
    n = str(node).lower()
    return {
        "role_br": 1.0 if "br-" in n else 0.0,
        "role_cr": 1.0 if "cr-" in n else 0.0,
        "role_fw": 1.0 if "fw" in n else 0.0,
        "role_traffic": 1.0 if "traffic-vm" in n else 0.0,
        "role_service": 1.0 if "service-vm" in n else 0.0,
        "role_monitor": 1.0 if "monitor-vm" in n else 0.0,
    }
