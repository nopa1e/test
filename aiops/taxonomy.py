...
def infer_category_from_evidence(
    ...
    It uses mechanism-level feature names and node roles instead of final
    business symptoms.  Directional checks (e.g. BGP peer_up < 1) avoid the
    common mistake of treating every BGP metric as a session-down event.
    It always returns a legal pair.
    """
    ...
    top_nodes = [str(n.get("network_element_id", "")) for n in (node_scores or [])]
    fw_top = any(("-fw" in n or n.endswith("fw")) for n in top_nodes[:3])
    service_top = any(("-service-vm-" in n) for n in top_nodes[:3])
    ...
    # --- fallbacks by node role ---
    if fw_top:
        return "firewall", "cpu_pressure"
    if service_top:
        return "service", "web_slow"
    if any(("br-" in n or "cr-" in n) for n in top_nodes[:3]):
        return "link", "loss"
    return "link", "delay"
